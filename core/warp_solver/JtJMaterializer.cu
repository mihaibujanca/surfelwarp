...
#pragma unroll
        ...
        __device__ __forceinline__ void computeSmoothJtJBlock_bk(
...
#pragma unroll
            ...
            TwistGradientOfScalarCost twist_gradient_i[3], twist_gradient_j[3];
            ...
            twist_gradient_i[1].rotation = make_float3(r.y, -r.x, 0.0f);
            twist_gradient_i[1].translation = make_float3(0.0f, 0.0f, 1.0f);
            twist_gradient_j[1].rotation = make_float3(-s.y, s.x, 0.0f);
            twist_gradient_j[1].translation = make_float3(0.0f,  0.0f, -1.0f);

            twist_gradient_i[2].rotation = make_float3(-r.z, 0.0f, r.x);
            twist_gradient_i[2].translation = make_float3(0.0f, 1.0f, 0.0f);
            twist_gradient_j[2].rotation = make_float3(s.z, 0.0f, -s.x);
            twist_gradient_j[2].translation = make_float3( 0.0f, -1.0f,  0.0f);


            //The order of two terms
            const float* jacobian_encoded_i[3];
            const float* jacobian_encoded_j[3];
            for(int idx = 0; idx < 3; idx++) {
                if (node_i == node_ij.x) {
                    jacobian_encoded_i[idx] = (const float *) (&twist_gradient_i[idx]);
                    jacobian_encoded_j[idx] = (const float *) (&twist_gradient_j[idx]);
                } else {
                    jacobian_encoded_i[idx] = (const float *) (&twist_gradient_j[idx]);
                    jacobian_encoded_j[idx] = (const float *) (&twist_gradient_i[idx]);
                }
            }
#pragma unroll
            for(int idx = 0; idx < 3; idx++)
...
#pragma unroll
                for (int jac_row = 0; jac_row < 6; jac_row++) {
                    shared_jtj_blks[6 * jac_row + 0][threadIdx.x] += weight_square * jacobian_encoded_i[idx][0] * jacobian_encoded_j[idx][jac_row];
                    shared_jtj_blks[6 * jac_row + 1][threadIdx.x] += weight_square * jacobian_encoded_i[idx][1] * jacobian_encoded_j[idx][jac_row];
                    shared_jtj_blks[6 * jac_row + 2][threadIdx.x] += weight_square * jacobian_encoded_i[idx][2] * jacobian_encoded_j[idx][jac_row];
                    shared_jtj_blks[6 * jac_row + 3][threadIdx.x] += weight_square * jacobian_encoded_i[idx][3] * jacobian_encoded_j[idx][jac_row];
                    shared_jtj_blks[6 * jac_row + 4][threadIdx.x] += weight_square * jacobian_encoded_i[idx][4] * jacobian_encoded_j[idx][jac_row];
                    shared_jtj_blks[6 * jac_row + 5][threadIdx.x] += weight_square * jacobian_encoded_i[idx][5] * jacobian_encoded_j[idx][jac_row];
                }
            }
        }
...
void surfelwarp::JtJMaterializer::BuildMaterializedJtJNondiagonalBlocksGlobalIteration(cudaStream_t stream) {
    ...
    dim3 blk(device::warp_size);
    dim3 grid(num_nodepairs);
//	same computation as computeJtJNonDiagonalBlockGlobalIterationKernel
    device::computeJtJNonDiagonalBlockKernel<<<grid, blk, 0, stream>>>(
            m_nodepair2term_map,
                    m_term2jacobian_map,
                    m_nondiag_blks.Ptr(),
