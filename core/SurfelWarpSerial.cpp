...
void surfelwarp::SurfelWarpSerial::Process(const cv::Mat* rgb, const cv::Mat* depth, bool compute_pose) {
    ...
    else
    {
        ...
        m_image_processor->ProcessFrameStreamed(observation, m_frame_idx, rgb, depth);
...
void surfelwarp::SurfelWarpSerial::saveCameraObservations(
    ...
    sprintf(frame_idx_str, "%06d", m_frame_idx);
    std::string raw_mask_name = "frame-";
    raw_mask_name += std::string(frame_idx_str);
    raw_mask_name += ".mask.png";
    Visualizer::SaveRawSegmentMask(observation.foreground_mask, (save_dir / raw_mask_name).string());
...
bool surfelwarp::SurfelWarpSerial::shouldDoReinit() const {
    ...
    return m_frame_idx > 0 && (m_frame_idx % period) == 0;
}
